...
create table Repeater (id bigint generated by default as identity, createdTs timestamp, modifiedTs timestamp, uid varchar(255) not null unique, version bigint, cron varchar(255) not null, name varchar(255) not null, times integer, primary key (id))


ALTER TABLE Expense add column iban varchar(255);

// 2017-07-21

alter table Expense add column fileDate date(8);
alter table Expense add column fileName varchar(512);
